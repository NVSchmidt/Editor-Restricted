...
define([
  ...
  function(declare, lang, array, html, esriBundle, on, query, Json, Deferred, aspect, all,
    ...
    Extent, Point, Identity, rendererJsonUtils, Graphic, Button, editUtils, FilterEditor, RelatedRecordsEditor) {
	var current_user = "";
	var portal = "";
	var current_window;
      ...
      onOpen: function() {
        ...
        */
		...
		require(["esri/arcgis/Portal"], function(arcgisPortal) {
			portal = new arcgisPortal.Portal("http://maps.lrp.ds.usace.army.mil/arcgis/");
			portal.signIn().then(function (loggedInUser){
			//console.log("Signed in to the portal as: ", loggedInUser);
			current_user = loggedInUser;
			var approvedUsers = "<div><ul>"
		 current_window.config.users.forEach(function(el){
			 approvedUsers += "<li><p>" + el + "</p></li>"
		 });
		 approvedUsers += "</ul></div>"
		 //this.usersNode.innerHTML = approvedUsers;
		 if(current_window.config.users.includes(current_user.username)){
			//this.statusNode.innerHTML = "<h4>You're on the list!</h4>";
			current_window._init();
			current_window._jimuLayerInfos = LayerInfos.getInstanceSync(current_window.map, current_window.map.itemInfo);
			var timeoutValue;
			if(current_window.appConfig.theme.name === "BoxTheme") {
				timeoutValue = 1050;
				current_window.loading.show();
			} else {
				timeoutValue = 1;
			}
			setTimeout(lang.hitch(current_window, function() {
				if(!current_window.loading.hidden) {
					current_window.loading.hide();
				}
				current_window.widgetManager.activateWidget(current_window);
				current_window._createEditor();
			}), timeoutValue);

			//prepare tableInfosParam data for relatedRecordsEditor
			current_window._getTableInfosParam().then(lang.hitch(current_window, function(tableInfosParam) {
				current_window.tableInfosParam = tableInfosParam;
				current_window.tableInfosParamClone = current_window._cloneLayerOrTableInfosParam(current_window.tableInfosParam);
				current_window._tableInfoParamDef.resolve();
			}));
		}
		 else{
		 ...
		 //this.usernameNode.innerHTML = current_user.fullName + "(username: " + current_user.username + ")";
      },
